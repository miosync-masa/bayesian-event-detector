...
def calculate_dynamic_sync_numpyro(series_a: jnp.ndarray, series_b: jnp.ndarray, 
    ...
    Args:
        series_a: 第1のイベント系列
        series_b: 第2のイベント系列
        window: スライディングウィンドウサイズ
        lag_window: 各ウィンドウ内の最大ラグ
        
    Returns:
        time_points: 時間インデックス
        sync_rates: 時間変化する同期率
        optimal_lags: 各時点での最適ラグ
    """
    T = len(series_a)
    sync_rates = []
    optimal_lags = []
    ...
    for t in range(T - window + 1):
        # ウィンドウ内データの抽出
        window_a = series_a[t:t+window]
        window_b = series_b[t:t+window]
        
        # 同期プロファイル計算
        _, sync_values = sync_profile_jax(window_a, window_b, lag_window)
        
        # 最大同期率とそのラグを記録
        max_idx = jnp.argmax(sync_values)
        max_sync = sync_values[max_idx]
        optimal_lag = max_idx - lag_window
        
        sync_rates.append(float(max_sync))
        optimal_lags.append(int(optimal_lag))
    
    time_points = jnp.arange(window//2, T - window//2 + 1)
    return np.array(time_points), np.array(sync_rates), np.array(optimal_lags)

# ===============================
# 動的同期の可視化（PyMC版と同一）
# ===============================
def plot_dynamic_sync_numpyro(time_points, sync_rates, optimal_lags, 
                             series_names=None, title=None):
    """
    時変同期率と最適ラグのプロット（PyMC版と完全互換）
    """
    fig, ax1 = plt.subplots(figsize=(10, 4))
    
    # 同期率のプロット
    ax1.plot(time_points, sync_rates, label='σₛ Sync Rate', 
             color='royalblue', linewidth=2)
    ax1.set_ylabel('σₛ Sync Rate', fontsize=12)
    ...
    # 最適ラグのプロット（右軸）
    ax2 = ax1.twinx()
    ax2.plot(time_points, optimal_lags, label='Optimal Lag', 
             color='darkorange', linestyle='--', linewidth=2)
    ax2.set_ylabel('Optimal Lag', fontsize=12)
    ax2.legend(loc='upper right')
    
    # タイトル設定
    if title is None and series_names:
        title = f"Dynamic Synchronization ({series_names[0]}↔{series_names[1]})"
    elif title is None:
        title = "Dynamic Synchronization (σₛ) and Optimal Lag"
...
# レジーム変化の動的可視化
# ===============================
def plot_regime_transitions_numpyro(data: jnp.ndarray, 
                                   regime_labels: np.ndarray,
                                   features: Dict[str, jnp.ndarray],
                                   series_name: str = "Series"):
    """
    レジーム遷移の動的可視化（PyMC版の拡張）
    """
    ...
    for regime in range(n_regimes):
        ...
        if len(regime_times) > 0:
            ...
            for i in range(len(regime_times)):
                if i == 0 or regime_times[i] != regime_times[i-1] + 1:
                    start = regime_times[i]
                    if i < len(regime_times) - 1:
                        j = i + 1
                        while j < len(regime_times) and regime_times[j] == regime_times[j-1] + 1:
                            j += 1
                        end = regime_times[j-1]
                    else:
                        end = regime_times[i]
                    ax1.axvspan(start, end, alpha=0.3, color=colors[regime])
    
    ax1.plot(data_np, 'k-', linewidth=1.5, label=series_name)
    ax1.set_ylabel('Value', fontsize=12)
    ax1.set_title(f'{series_name}: Regime Transitions', fontsize=14)
    ax1.grid(True, alpha=0.3)
    ax1.legend()
    
    # 2. テンションスカラー（ρT）の推移
    rho_t = np.array(features['rho_t'])
    ax2.plot(rho_t, 'g-', linewidth=1.5, label='ρT (Tension)')
    ax2.set_ylabel('Tension Scalar ρT', fontsize=12)
    ax2.set_title('Market Tension Evolution', fontsize=14)
    ax2.grid(True, alpha=0.3)
    ax2.legend()
    ...
    for regime in range(n_regimes):
        mask = regime_labels == regime
        mean_rho = np.mean(rho_t[mask])
        ax2.axhline(y=mean_rho, color=colors[regime], linestyle='--', 
                   alpha=0.7, label=f'Regime {regime+1} Mean')
    
    # 3. レジームインジケーター
    ax3.plot(regime_labels, 'o-', markersize=3)
    ax3.set_ylabel('Regime', fontsize=12)
    ax3.set_xlabel('Time Step', fontsize=12)
    ax3.set_title('Regime Indicator', fontsize=14)
    ax3.set_yticks(range(n_regimes))
    ax3.set_yticklabels([f'Regime {i+1}' for i in range(n_regimes)])
...
# マルチスケール解析結果の可視化
# ===============================
def plot_multiscale_analysis_numpyro(scale_features: Dict[int, Dict[str, np.ndarray]],
                                    scale_breaks: List[Tuple[int, List[int]]],
                                    data: jnp.ndarray = None):
    """
    マルチスケール解析結果の統合可視化
    """
    scales = sorted(scale_features.keys())
    n_scales = len(scales)
    
    fig, axes = plt.subplots(n_scales + 1, 1, figsize=(15, 3 * (n_scales + 1)), 
                            sharex=True)
    
    # 元データのプロット（あれば）
    if data is not None:
        ax = axes[0]
        ax.plot(np.array(data), 'k-', linewidth=1, alpha=0.7)
        ax.set_ylabel('Original Data')
        ax.set_title('Multi-Scale Lambda³ Analysis', fontsize=16)
        ax.grid(True, alpha=0.3)
        
        # スケールブレイクをマーク
        for scale, breaks in scale_breaks:
            for break_idx in breaks:
                ax.axvline(x=break_idx, color='red', linestyle='--', 
                          alpha=0.5, linewidth=1)
    
    # 各スケールでの特徴量
    for i, scale in enumerate(scales):
        ax = axes[i + 1] if data is not None else axes[i]
        features = scale_features[scale]
        
        # ローリング標準偏差
        rolling_std = features['rolling_std']
        ax.plot(rolling_std, label=f'σ (scale={scale})', linewidth=1.5)
        
        # 該当スケールでのブレイク
        for s, breaks in scale_breaks:
            if s == scale:
                for break_idx in breaks:
                    ax.axvline(x=break_idx, color='red', linestyle='--', 
                              alpha=0.7, linewidth=2)
                    ax.text(break_idx, ax.get_ylim()[1] * 0.9, 
                           f'Break', rotation=90, verticalalignment='bottom')
...
# 全ペア解析のサマリー機能
# ===============================
def create_pairwise_summary_numpyro(pair_results: Dict[str, Any],
                                   series_names: List[str]):
    """
    全ペア解析結果の包括的サマリー作成（PyMC版と互換）
    """
    print("\n" + "="*60)
    print("COMPREHENSIVE PAIRWISE ANALYSIS SUMMARY")
    print("="*60)
    
    if 'interaction_effects' not in pair_results:
        print("No pairwise analysis results available.")
        return
    
    interaction_effects = pair_results['interaction_effects']
    sync_profiles = pair_results.get('sync_profiles', {})
    causality_results = pair_results.get('causality_results', {})
    
    # 1. 相互作用効果のサマリー
    print("\n📊 INTERACTION EFFECTS SUMMARY")
    print("-" * 40)
    print("Significant interactions (|β| > 0.1):")
    
    significant_interactions = []
    for (name_a, name_b), effects in interaction_effects.items():
        for effect_type, value in effects.items():
            if abs(value) > 0.1:
                significant_interactions.append((value, name_a, name_b, effect_type))
    
    significant_interactions.sort(key=lambda x: abs(x[0]), reverse=True)
    
    for value, name_a, name_b, effect_type in significant_interactions[:10]:
        arrow = "→" if "to" in effect_type else "↔"
        print(f"  {name_a} {arrow} {name_b} ({effect_type}): β = {value:.3f}")
    
    # 2. 同期関係のサマリー
    if sync_profiles:
        print("\n🔗 SYNCHRONIZATION SUMMARY")
        print("-" * 40)
        print("Top synchronized pairs:")
        
        sync_pairs = []
        for (name_a, name_b), profile in sync_profiles.items():
            max_sync = profile['max_sync']
            optimal_lag = profile['optimal_lag']
            sync_pairs.append((max_sync, name_a, name_b, optimal_lag))
        
        sync_pairs.sort(reverse=True)
        
        for max_sync, name_a, name_b, lag in sync_pairs[:5]:
            lag_str = f"lag={lag:+d}" if lag != 0 else "no lag"
            print(f"  {name_a} ↔ {name_b}: σₛ = {max_sync:.3f} ({lag_str})")
    
    # 3. 因果関係のサマリー
    if causality_results:
        print("\n🎯 CAUSALITY SUMMARY")
        print("-" * 40)
        print("Strongest causal relationships:")
        
        causal_pairs = []
        for (name_a, name_b), causality_data in causality_results.items():
            for direction, lag_profile in causality_data.items():
                if isinstance(lag_profile, dict):
                    max_causality = max(lag_profile.values()) if lag_profile else 0
                    optimal_lag = max(lag_profile.items(), key=lambda x: x[1])[0] if lag_profile else 0
                    causal_pairs.append((max_causality, direction, optimal_lag))
        
        causal_pairs.sort(reverse=True)
        
        for causality, direction, lag in causal_pairs[:5]:
            if causality > 0.1:
                print(f"  {direction}: p = {causality:.3f} at lag {lag}")
    ...
    # ノード中心性の計算
    n = len(series_names)
    in_strength = {name: 0.0 for name in series_names}
    out_strength = {name: 0.0 for name in series_names}
    
    for (name_a, name_b), effects in interaction_effects.items():
        for effect_type, value in effects.items():
            if 'to' in effect_type and '_pos' in effect_type:
                # name_b → name_a
                parts = effect_type.split('_to_')
                if len(parts) == 2:
                    out_strength[name_b] += abs(value)
                    in_strength[name_a] += abs(value)
    
    print("Node centrality (by interaction strength):")
    centrality_scores = []
    for name in series_names:
        total_strength = in_strength[name] + out_strength[name]
        centrality_scores.append((total_strength, name, in_strength[name], out_strength[name]))
    
    centrality_scores.sort(reverse=True)
    
    for total, name, in_str, out_str in centrality_scores:
        if total > 0:
            print(f"  {name}: total={total:.3f} (in={in_str:.3f}, out={out_str:.3f})")
...
# PyMC互換レポート生成の拡張
# ===============================
def create_comprehensive_report_numpyro_extended(results: Dict[str, Any]):
    """
    PyMC版と完全互換の拡張レポート生成
    """
    # 既存のレポート生成を呼び出し
    create_comprehensive_report_numpyro(results)
    ...
    if 'analysis_results' in results:
        ...
        # 動的同期解析結果
        if 'dynamic_sync_results' in analysis_results:
            print("\n📈 DYNAMIC SYNCHRONIZATION ANALYSIS")
            print("-" * 40)
            
            dyn_sync = analysis_results['dynamic_sync_results']
            for pair_name, dyn_data in dyn_sync.items():
                mean_sync = np.mean(dyn_data['sync_rates'])
                std_sync = np.std(dyn_data['sync_rates'])
                print(f"  {pair_name}:")
                print(f"    Mean sync rate: {mean_sync:.3f} ± {std_sync:.3f}")
                print(f"    Sync range: [{np.min(dyn_data['sync_rates']):.3f}, "
                      f"{np.max(dyn_data['sync_rates']):.3f}]")
        ...
        if 'multiscale_results' in analysis_results:
            ...
            ms_results = analysis_results['multiscale_results']
            if 'scale_breaks' in ms_results:
                print("  Scale break summary:")
                for scale, breaks in ms_results['scale_breaks']:
                    if breaks:
                        print(f"    Scale {scale}: {len(breaks)} breaks at {breaks[:3]}...")
        
        # 条件付き同期結果
        if 'conditional_sync' in analysis_results:
            print("\n🎲 CONDITIONAL SYNCHRONIZATION")
            print("-" * 40)
            
            cond_sync = analysis_results['conditional_sync']
            for condition_name, sync_value in cond_sync.items():
                print(f"  {condition_name}: σₛ = {sync_value:.3f}")        


# ===============================
# Lambda3MultiScaleAnalyzerのNumPyro版
# ===============================
class Lambda3MultiScaleAnalyzerNumPyro:
    """
    マルチスケールLambda³解析（NumPyro版）
    """
    def __init__(self, scales=[5, 10, 20, 50]):
        self.scales = scales
        self.scale_features = {}
    
    def extract_features(self, data: jnp.ndarray):
        """各スケールでの特徴抽出"""
        self.scale_features = {}
        data_np = np.array(data)
        ...
        for w in self.scales:
            # スケールwでのローリング標準偏差
            rolling_std = np.array([
                np.std(data_np[max(0, i-w):i+1]) 
                for i in range(len(data_np))
            ])
            
            # ジャンプ検出
            diff = np.diff(data_np, prepend=data_np[0])
    ...
    def detect_scale_breaks(self, threshold=1.5):
        ...
        for w, feats in self.scale_features.items():
            ...
            # 閾値を超えるピークを検出
            peaks = np.where(std > mean_std + threshold * std_std)[0]
            if len(peaks) > 0:
...
# メイン解析パイプラインの拡張
# ===============================
def main_lambda3_numpyro_analysis_extended(csv_path: str = None, 
    ...
    """
    拡張版Lambda³ NumPyro解析パイプライン（PyMC完全互換）
    """
    # 基本解析の実行
    ...
    if run_advanced_analysis:
        ...
        series_dict = results['series_dict']
        ...
        # 1. 全ペア解析（PyMC版と同じインターフェース）
        if analyze_all_pairs and len(series_names) >= 2:
            ...
            advanced_results.update(pair_results)
            
            # ペア解析のサマリー作成
            create_pairwise_summary_numpyro(pair_results, series_names)
        
        # 2. 動的同期解析
        print("\n🔄 Dynamic Synchronization Analysis...")
        dynamic_sync_results = {}
        
        if len(series_names) >= 2:
            # 最初の数ペアについて動的同期を計算
            from itertools import combinations
            pairs = list(combinations(series_names, 2))[:3]  # 最初の3ペア
            
            for name_a, name_b in pairs:
                # イベント系列の取得
                events_a = features_dict[name_a]['delta_lambda_pos']
                events_b = features_dict[name_b]['delta_lambda_pos']
                
                # 動的同期計算
                time_points, sync_rates, optimal_lags = calculate_dynamic_sync_numpyro(
                    events_a, events_b, window=20, lag_window=10
                )
                
                pair_name = f"{name_a}↔{name_b}"
                dynamic_sync_results[pair_name] = {
                ...
                # プロット
                plot_dynamic_sync_numpyro(
                    time_points, sync_rates, optimal_lags,
                    series_names=[name_a, name_b]
                )
        
        advanced_results['dynamic_sync_results'] = dynamic_sync_results
        
        # 3. マルチスケール解析
        print("\n📊 Multi-Scale Analysis...")
        from Lambda3MultiScaleAnalyzer import Lambda3MultiScaleAnalyzerNumPyro
        
        ms_analyzer = Lambda3MultiScaleAnalyzerNumPyro(scales=[5, 10, 20, 50])
        multiscale_results = {}
        ...
        for i, (name, data) in enumerate(series_dict.items()):
            ...
            # スケール別特徴抽出
            scale_features = ms_analyzer.extract_features(data)
            scale_breaks = ms_analyzer.detect_scale_breaks(threshold=1.5)
            
            multiscale_results[name] = {
                'scale_features': scale_features,
                'scale_breaks': scale_breaks
            }
            
            # 可視化
            plot_multiscale_analysis_numpyro(
                scale_features, scale_breaks, data
            )
        
        advanced_results['multiscale_results'] = multiscale_results
        
        # 4. レジーム遷移の動的可視化
        if 'regime_results' in results.get('analysis_results', {}):
            regime_results = results['analysis_results']['regime_results']
            if 'regime_labels' in regime_results:
                print("\n🎯 Visualizing Regime Transitions...")
                
                for i, name in enumerate(series_names[:2]):  # 最初の2系列
                    plot_regime_transitions_numpyro(
                        series_dict[name],
        ...
        if len(series_names) >= 2:
            ...
            for name_a, name_b in pairs[:3]:
                events_a = features_dict[name_a]['delta_lambda_pos']
                events_b = features_dict[name_b]['delta_lambda_pos']
                rho_t_a = features_dict[name_a]['rho_t']
                
                # 高テンション期間の同期率
                high_tension_sync = lambda3_conditional_sync_numpyro(
                    events_a, events_b, rho_t_a,
                    condition_threshold=float(jnp.median(rho_t_a))
                )
                
                conditional_sync_results[f"{name_a}↔{name_b} (high tension)"] = high_tension_sync
        
        advanced_results['conditional_sync'] = conditional_sync_results
        
        # 結果の統合
        if 'analysis_results' not in results:
            results['analysis_results'] = {}
        results['analysis_results'].update(advanced_results)
    
    # 拡張レポートの生成
    create_comprehensive_report_numpyro_extended(results)
    
    # PyMC互換の出力生成
    generate_pymc_compatible_output(results)
...
if __name__ == "__main__":
    ...
    # 拡張版解析の実行
    results = main_lambda3_numpyro_analysis_extended(
        config=L3ConfigNumPyro(
            num_samples=8000,
            num_warmup=8000,
            num_chains=2
