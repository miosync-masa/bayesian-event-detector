...
def analyze_multi_asset_regimes(
    ...
    """Analyze regimes across multiple financial assets with Lambda³ approach."""
    print(f"\n{'='*60}")
    ...
    for i, asset_a in enumerate(asset_names):
        for j, asset_b in enumerate(asset_names):
            if i != j:
                ...
                regime_sync_matrix[i, j] = agreement
            else:
                regime_sync_matrix[i, j] = 1.0

    plot_regime_synchronization_matrix(regime_sync_matrix, asset_names)
...
def analyze_complete_causality(
    ...
    """
    統合因果分析 - 基本構造因果関係の包括解析

    Lambda³理論: 構造テンソル空間における基本因果関係の完全解析
    構造テンソル∆ΛC pulsationsの因果パターンを解析し、
    progression vectors (ΛF) と tension scalars (ρT) の
    相互作用による因果関係の emergence を定量化する。
    ...
    Dict[str, any]
        統合因果分析結果
        - basic_causality: 基本構造因果関係
        - summary: 因果関係統計サマリー
        - causality_matrix: 因果関係行列
        - temporal_causality: 時間依存因果パターン
    """
    print(f"\n{'='*60}")
    print("LAMBDA³ COMPLETE CAUSALITY ANALYSIS")
    print("構造テンソル空間における因果関係の完全解析")
    print(f"{'='*60}")
    print(f"解析系列: {', '.join(series_names)}")
    print(f"遅延窓幅: {lag_window}")

    causality_results = {}

    # === 1. 基本構造因果関係検出 ===
    print(f"\n基本構造因果関係を分析中...")
    basic_causality = detect_basic_structural_causality(
        features_dict, series_names, lag_window
    )
    causality_results['basic_causality'] = basic_causality

    # === 2. 因果関係行列の構築 ===
    print(f"因果関係行列を構築中...")
    causality_matrix = _build_causality_matrix(basic_causality, series_names)
    causality_results['causality_matrix'] = causality_matrix

    # === 3. 時間依存因果パターン分析 ===
    print(f"時間依存因果パターンを分析中...")
    temporal_causality = _analyze_temporal_causality_patterns(
        basic_causality, lag_window
    )
    causality_results['temporal_causality'] = temporal_causality
    ...
    for direction, pattern in basic_causality.items():
        if pattern and isinstance(pattern, dict) and pattern:
            max_causality = max(pattern.values())
            mean_causality = np.mean(list(pattern.values()))
            all_causalities.append(max_causality)
    ...
    causality_results['summary'] = causality_summary
    causality_results['directional_strengths'] = directional_strengths

    # === 6. 構造テンソル因果特性 ===
    structural_causality_metrics = _compute_structural_causality_metrics(
        features_dict, series_names, basic_causality
    )
    causality_results['structural_metrics'] = structural_causality_metrics

    # === 7. 結果表示 ===
    _display_causality_results(causality_summary, directional_strengths,
